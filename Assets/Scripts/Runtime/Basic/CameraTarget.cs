...
public class CameraTarget : MonoBehaviour
{
    [Header("Static")]
    [Space]
    [SerializeField] private float threshold;//位移阈值
    ...
    IEnumerator UpDatePosition()
    {
        while (Active)
        {
            ...
            newPos.x = Mathf.Clamp(newPos.x, -threshold + heroTransform.position.x, threshold + heroTransform.position.x);
            newPos.y = Mathf.Clamp(newPos.y, -threshold + heroTransform.position.y, threshold + heroTransform.position.y);
